...
const setEl = (el: Element | ComponentPublicInstance | null, i: number) => {
...
/* Hovered item (and its tooltip) stacks above magnified neighbours */
.dock-item:hover { z-index: 20; }
.glyph {
  display: block;
...
.tip {
  position: absolute;
  bottom: calc(100% + 40px); /* sit clear above the magnified icon */
  left: 50%;
...
/* little caret under the tooltip */
.tip::after {
  content: '';
  position: absolute;
  top: 100%;
  ...
  border-top-color: rgba(30, 34, 42, 0.95);
}
